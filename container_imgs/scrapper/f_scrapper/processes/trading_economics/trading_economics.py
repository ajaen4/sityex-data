...
def trading_econ_process() -> None:
    ...
    country_income_cfg = TradingEconScrapeConfigs.COUNTRY_INCOME_TAX.value
    run_scrape(content_writer, trading_econ_extractor, country_income_cfg)

    social_security_cfg = (
        TradingEconScrapeConfigs.COUNTRY_SOCIAL_SECURITY_RATE.value
    )
    run_scrape(content_writer, trading_econ_extractor, social_security_cfg)

    sales_tax_cfg = TradingEconScrapeConfigs.COUNTRY_SALES_TAX_RATE.value
    run_scrape(content_writer, trading_econ_extractor, sales_tax_cfg)

    job_vacancies_cfg = TradingEconScrapeConfigs.JOB_VACANCIES.value
