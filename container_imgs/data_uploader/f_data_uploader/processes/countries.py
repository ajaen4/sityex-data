...
def upload_countries_to_firestore() -> None:
    ...
    countries_info = s3_client.read_dics(
        cfg.DATA_BUCKET_NAME, countries_info_path
    )

    collection_ref = fire_client.collection("countries")
    for index, doc in enumerate(countries_info):
        logger.info(
            f'Uploading doc number {index}, country_name: {doc["name"]}...'
        )
...
def create_country_doc(doc: dict) -> dict[str, Any]:
    country_doc = dict()
...
def get_indicators(country_code: str) -> list:
    ...
    indicators_details = s3_client.read_dics(
        cfg.DATA_BUCKET_NAME, country_ind_path
    )
...
def upload_countries_cost_map():
    cost_map_path = f"bronze/countries/costs_ids_index/{cfg.FORMATTED_DATE}/costs_ids_index.csv"
