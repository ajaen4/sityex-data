...
def upload_cities_to_firestore() -> None:
    cities_file_dir = (
    ...
    show_cities_dir = "maps/cities_to_show.csv"
    cities_to_show = s3_client.read_dics(cfg.DATA_BUCKET_NAME, show_cities_dir)
    cities_to_show = [
        remove_diacritics(row["city_name"]) for row in cities_to_show
    ]

    cities_comm_links_dir = "maps/community_links.csv"
    cities_community_links = s3_client.read_dics(
        cfg.DATA_BUCKET_NAME, cities_comm_links_dir
    )
...
def create_city_doc(s3_client: S3, doc: dict) -> dict[str, Any]:
    city_doc = dict()
    ...
    if prices_file_path:
        city_prices = s3_client.read_dics(
            cfg.DATA_BUCKET_NAME, prices_file_path
        )
    ...
    weather_infos = s3_client.read_dics(
        cfg.DATA_BUCKET_NAME, weather_file_path
    )
...
def create_index_entry(doc: dict) -> dict[str, Any]:
    index_entry = dict()
...
def upload_cities_cost_map():
    cost_map_path = f"bronze/cities/costs_ids_index/{cfg.FORMATTED_DATE}/costs_ids_index.csv"
