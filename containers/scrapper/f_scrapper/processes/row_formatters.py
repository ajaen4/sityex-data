...
def get_rows_country_ranking(rows):
    data = []
    ...
    for row in rows:
        ...
        cols = [
            float(ele.text.strip()) if is_number(ele.text.strip()) else ele.text.strip()
            for ele in cols
        ]
...
def get_rows_city_ranking(rows):
    data = []
    extra_codes_map = fetch_extra_codes()
    ...
    for row in rows:
        cols = row.find_all("td")
        cols = [
            float(ele.text) if is_number(ele.text) else ele.text.strip('"').strip()
            for ele in cols
        ]
...
def get_rows_price_country(rows):
    data = []
    extra_codes_map = fetch_extra_codes()
...
def fetch_extra_codes():
    ...
    for extra_code in extra_country_codes:
        extra_codes_map[extra_code["country_name"]] = extra_code["country_code"]
